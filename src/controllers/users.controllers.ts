...
export const refreshTokenController = async (
  ...
  const result = await usersServices.refreshToken({ user_id, refresh_token, verify, exp })
  return res.json({
...
export const verifyForgotPasswordController = async (
  ...
  return res.json({
    messsage: USERS_MESSAGES.VERIFY_FORGOT_PASSWORD_SUCCESS
  })
}
...
export const getMeController = async (req: Request, res: Response, next: NextFunction) => {
  ...
  return res.json({
    messsage: USERS_MESSAGES.GET_ME_SUCCESS,
    result: user
  })
}
...
export const unFollowController = async (req: Request<UnFollowUserReqParams>, res: Response, next: NextFunction) => {
  const { user_id } = req.decoded_authorization as TokenPayload
